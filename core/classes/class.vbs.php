 ...
 * Class Vbs
 *
 * Provides utility functions for interacting with the Windows operating system using VBScript.
 * Includes methods for counting files and folders, retrieving default and installed browsers,
...
class Vbs
{
    ...
    public static function countFilesFolders(string $path): int|false
    {
        ...
        $content .= 'Sub CountFiles(ByVal path)' . PHP_EOL;
        ...
        $content .= '        CountFiles(subFld.Path)' . PHP_EOL;
        $content .= '    Next' . PHP_EOL;
        $content .= 'End Sub' . PHP_EOL . PHP_EOL;
    ...
    public static function getListProcs(array $vbsKeys): array|false
    {
        ...
        $content = substr($content, 0, -strlen($sep) - 1) . ')' . PHP_EOL;
        ...
        $result = array_filter($result, function ($line) {
            return trim($line) !== self::END_PROCESS_STR;
        });
     ...
     * @return string|null The startup path, or null on failure.
     */
    public static function getStartupPath(?string $file = null): ?string
    {
        ...
        return $file !== null ? $path . '/' . $file : $path;
    }
    ...
    public static function createShortcut(string $savePath): bool
    {
        ...
        $content = 'Dim objShell, objFso, objResultFile, objShortcut' . PHP_EOL . PHP_EOL;
        $content .= 'Set objShell = Wscript.CreateObject("Wscript.Shell")' . PHP_EOL;
        $content .= 'Set objFso = CreateObject("scripting.filesystemobject")' . PHP_EOL;
        $content .= 'Set objResultFile = objFso.CreateTextFile("' . $resultFile . '", True)' . PHP_EOL . PHP_EOL;
    ...
    public static function getTmpFile(string $ext, ?string $customName = null): string
    {
        ...
        $filename = (!empty($customName) ? $customName . '-' : '') . Util::random() . $ext;
        return Util::formatWindowsPath($bearsamppCore->getTmpPath() . '/' . $filename);
    }
    ...
    public static function exec(string $basename, string $resultFile, string $content, int|bool $timeout = true): array|false
    {
        ...
        $timeoutDuration = is_numeric($timeout) ? (int)$timeout : ($timeout === true ? $bearsamppConfig->getScriptsTimeout() : false);
        $maxtime = time() + $timeoutDuration;
        $noTimeout = $timeoutDuration === false;
        $result = false;

        while ($result === false || empty($result)) {
            if (file_exists($checkFile)) {
                $check = file($checkFile, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
                if (!empty($check) && trim($check[0]) === self::END_PROCESS_STR) {
                    $result = file($resultFile, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
                    break;
                }
            }
            if (!$noTimeout && time() >= $maxtime) {
                break;
            }
            usleep(100000); // Sleep for 0.1 seconds to prevent CPU overuse
        }
