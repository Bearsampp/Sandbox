...
class ActionEditAlias
{
    ...
    public function __construct(array $args)
    {
        ...
        if (isset($args[0]) && !empty($args[0])) {
            ...
            if (preg_match('/^Alias \/' . preg_quote($args[0], '/') . ' "(.+)"/', $fileContent, $match)) {
                ...
                $this->wbWindow = $bearsamppWinbinder->createAppWindow(
                    sprintf($bearsamppLang->getValue(Lang::EDIT_ALIAS_TITLE), $this->initName),
                ...
                $this->wbInputName = $bearsamppWinbinder->createInputText(
                    $this->wbWindow, $this->initName, 105, 13, 150, null
                );
                ...
                $this->wbLabelExp = $bearsamppWinbinder->createLabel(
                    $this->wbWindow, sprintf($bearsamppLang->getValue(Lang::ALIAS_EXP_LABEL), $apachePortUri, $this->initName, $initDest), 15, 80, 470, 50
                );
